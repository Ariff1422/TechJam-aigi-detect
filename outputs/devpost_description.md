...
- **We chased the calibration hypothesis to its actual conclusion instead of stopping at the first negative result.** A single global threshold shift (raising the cutoff to the validation-optimal 0.64) made every degraded/OOD condition worse, and temperature scaling turned out to be mathematically incapable of changing accuracy at a fixed threshold at all — both dead ends. But an oracle test (the accuracy-maximizing threshold *for each specific distortion condition*, fit and evaluated on separate halves of the data) recovered 15-23 points of accuracy on the hardest conditions, including a 20.7pt recovery on CIFAKE — proof the gap really is substantially a calibration problem, just one that needs to know the input's degradation condition to fix, which nothing in the current inference pipeline does. That's honest, evidence-backed scoping for real future work (a degradation-aware calibration mechanism), not a guess.
