...
- **We also tested a fix that didn't pan out, and reported that plainly rather than quietly dropping it:** since AUC held up far better than fixed-threshold accuracy under distortion, we tried raising the classification threshold to the value that maximized validation accuracy (0.64). It helped clean/validation trivially but made every degraded and out-of-distribution condition worse, worst on the hardest ones (-4.2pt on held-out Gaussian noise, -1.0pt on CIFAKE) — confirming the real issue is the whole confidence distribution shifting under distortion, not one fixed threshold being misplaced.
